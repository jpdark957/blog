...
public class LoginController {
    ...
    @PostMapping(value = "/user/useradd")
    @ApiOperation("注册账号")
    public String reg(String username,String password, Map map) {
        String hashpw = BCrypt.hashpw(password, BCrypt.gensalt());
        boolean adduser = userService.adduser(username, hashpw);
        if (!adduser){
            map.put("regerror", "注册失败，账号已存在");
            return "comm/reg";
        }else{
            return "comm/login";
        }

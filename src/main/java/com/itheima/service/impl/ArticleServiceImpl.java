...
public class ArticleServiceImpl implements IArticleService {
    ...
    public PageInfo<Article> selectArticleWithPage(Integer page, Integer count) {
        ...
        for (int i = 0; i < articleList.size(); i++) {
            ...
            //查询封装分类标题到实体类一起返回给控制层
            Integer categoryId = articleList.get(i).getCategories();
            Category category = categoryMapper.queryCategoryByid(categoryId);
            articleList.get(i).setCategoryTitle(category.getTitle());
    ...
    public List<Article> searchArticle(String title) {
        List<Article> articleList = articleMapper.searchArticle(title);
        for (int i = 0; i <articleList.size() ; i++) {
            Integer categoryId = articleList.get(i).getCategories();
            Category category = categoryMapper.queryCategoryByid(categoryId);
            articleList.get(i).setCategoryTitle(category.getTitle());
